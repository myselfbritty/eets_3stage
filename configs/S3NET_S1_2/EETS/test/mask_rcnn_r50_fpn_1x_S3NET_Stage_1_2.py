...
data_root = 'data/EETS/'  #####Path to the root folder 
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'train/segms/EETS_train.json', #######Annotation file coco format for training dataset
    ...
    val=dict(
        ...
        ann_file=data_root + 'val/segms/EETS_val.json', #######Annotation file coco format for testing dataset
        img_prefix=data_root + 'val/images/', #######Images for train dataset
    ...
    test=dict(
        type=dataset_type,
        ####For testing foldwise
        ann_file=data_root + 'test/segms/EETS_val.json', #######Annotation file coco format for testing dataset
        img_prefix=data_root +'test/images/', #######Images for train dataset
        ####For testing seq 9 10
        # ann_file=data_root + 'test_crop/coco-annotations/instances_test_sub.json', #######Annotation file coco format for testing dataset
        # img_prefix=data_root +'test/images/', #######Images for testing dataset
        img_scale=(1333, 800),
...
log_level = 'INFO'
work_dir = './work_dirs/S3NET_Stage_1_2/EETS/' #####Path to the ouput folder
######Use load_from for training the new dataset 
load_from ='pre-trained-weights/basic_chkpoints/resized_weights_r50_eets_backbone_class10.pth' #####ImageNet weights resized to class 8 
