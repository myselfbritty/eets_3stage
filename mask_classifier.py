...
class make_classifier(tf.keras.Model):
    ...
    def __init__(self, num_classes = 10, WEIGHT_DECAY = 1e-4):
        ...
        self.dropout2 = LY.Dropout(0.35)

        self.dense2 = LY.Dense(1024, activation = 'linear', use_bias=True)
    ...
    def call(self, inputs, mode = 'margin', training = True):
        ...
        fp_o = self.concat([fp_2, fp_3, fp_4, fp_5])     # bs, 1024, 56, 56

        fp_o = tf.multiply(fp_o,mask_ip)
        fp_o = self.ga(fp_o)
        # fp_o = self.dropout1(fp_o, training=training)
        fp_o = self.dense1(fp_o)
        # fp_o = self.dropout2(fp_o, training=training)
        fp_o = self.dense2(fp_o)
        # return fp_o
        # fp_o = self.masking_layer(fp_o, mask_ip)
        # fp_o = self.dropout1(fp_o, training = training)
        # x = self.dense1(fp_o)
        
        op = self.arc_layer([fp_o,y], mode = mode)
        return op


# model = make_classifier(num_classes=10)
# ip1 = np.zeros((1,3,224,224))
# ip2 = np.zeros((1,1,56,56))
# ip3 = np.zeros((1,10))
# input_1 = tf.constant(ip1)
# input_2 = tf.constant(ip2)
# input_3 = tf.constant(ip3)
# op = model([input_1,input_2,input_3],mode = 'softmax').numpy()
# print(op.shape)